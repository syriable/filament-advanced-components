 ...
 * happens once per cell inside {@see getProgressData()}, so the Blade view
...
class MultiProgressColumn extends Column
{
    ...
    */
     ...
     *
     * When omitted entirely, the column falls back to its own state, so a
    ...
    */
    ...
    public function getSegments(): array
    {
        ...
        // Fall back to the column's state, so an Eloquent accessor (or JSON
        // cast attribute) returning a segments array needs no extra setup.
        if ($segments === null) {
            $state = $this->getState();
    ...
    public function getHeight(): string
    {
        ...
        return self::SIZE_HEIGHTS[$size] ?? self::SIZE_HEIGHTS['md'];
    }
    ...
    */
     ...
     * exactly once per cell so no math or color resolution happens in Blade.
     *
    ...
    {
        if ($segments === []) {
            return __('filament-advanced-components::multi-progress-column.empty');
        }
    ...
    /**
     * Makes `$segment`, `$segments` and `$total` injectable into every
     * user-provided closure, alongside Filament's defaults (`$record`,
     * `$state`, `$livewire`, ...).
     *
     * @return array<mixed>
     */
