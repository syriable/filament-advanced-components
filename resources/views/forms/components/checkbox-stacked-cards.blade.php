...
<x-dynamic-component :component="$fieldWrapperView" :field="$field">
    <div x-load
        ...
        <div
            {{ $getExtraAttributeBag()->grid($getColumns(), $gridDirection)->merge(
                    [
            ...
            @forelse ($options as $value => $label)
                <div wire:key="{{ $livewireKey }}.options.{{ $value }}"
                    ...
                    <x-filament::input.wrapper>
                        <label class="fi-fo-checkbox-list-option px-4 py-3 flex items-center justify-between">

                            <div class="fi-fo-checkbox-list-option-text flex items-center gap-4">
                                <input type="checkbox"
                            ...
                            @if ($hasExtra($value))
                                <div class="fi-fo-checkbox-list-option-label font-medium">
                                    {{ $getExtra($value) }}
                                </div>
                            @endif
                        </label>
