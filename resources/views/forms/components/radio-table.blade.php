...
<x-dynamic-component :component="$fieldWrapperView" :field="$field">
    <div x-data="{
        ...
        @foreach ($getOptions() as $value => $label)
            ...
            <label for="{{ $id . '-' . $value }}"
                class="p-4 overflow-hidden grid grid-cols-3 w-full border-x border-gray-300 first:rounded-t-md last:rounded-b-md first:border-t last:border-b border-b has-checked:border-primary-300 has-checked:bg-primary-50">
                ...
                @if ($hasDescription($value))
                    <p class="fi-fo-radio-label-description text-start text-gray-500 dark:text-gray-200">
                        {{ $getDescription($value) }}
                    </p>
                @endif
                @if ($hasExtra($value))
                    <div class="fi-fo-radio-label-description text-end text-gray-500 dark:text-gray-200">
                        {{ $getExtra($value) }}
                    </div>
                @endif
            </label>
